...
import { LoginRequestDTO, LoginResponseDTO } from '../session/sessions.dto';
...
export class AuthService {
  ...
  /* We stoppen het gebruikers id, email en rollen in de JWT payload.
    ...
    De nodige opties mee om de JWT te ondertekenen dienen we niet mee te geven, want reeds doorgegeven bij de registratie van de JwtModule.*/
  ...
  async login({ email, password }: LoginRequestDTO): Promise<string> {
    ...
    return this.signJwt(user); // Als alles klopt, ondertekenen we een JWT en geven deze terug.
  }
